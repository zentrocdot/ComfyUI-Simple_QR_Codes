...
class QRCodesSegnoLogo:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "width_height_logo": ("INT", {"default": 256, "min": 0, "max": 1024}),
    ...
    def qr_code_creation(self, text, width, height, error_correct, version,
             ...
             width_height_logo, image=None):
        '''Main node function. Create a QR code image.'''
        if image is not None:
            # Create a tensor from the image.
            image = tensor2pil(image)
        else:
            col = string2tuple(light)
            n,m = self.basewidth,self.basewidth
            image = Image.new('RGB', (n, m), col)
            #width_height_logo = self.basewidth
        logo = image
        # Adjust the logo image size.
        w_percent = width_height_logo / float(logo.size[0])
        h_size = int((float(logo.size[1]) * float(w_percent)))
        logo = logo.resize((width_height_logo, h_size), resample=3)
        # Create the QR code from the given text.
        QRimg = self.create_qr_code(text, error_correct, version,
        ...
        QRimg = QRimg.resize((width, height), resample=3)
        QRimg = QRimg.convert(mode='RGB')
        pos = (
